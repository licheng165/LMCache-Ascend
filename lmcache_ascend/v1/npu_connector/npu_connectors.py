...
class VLLMPagedMemLayerwiseNPUConnector(VLLMPagedMemLayerwiseGPUConnector):
    ...
    def _batched_to_gpu_head_token_wise_prepared(
        ...
        for layer_id, source_layer in enumerate(source.layers):
            ...
            # A cold bootstrap has no selection because the index prefix must
            # be resident before the indexer can select sparse tokens.
    ...
    def batched_to_gpu_head_token_wise(self, **kwargs):
        ...
        for layer_id in range(
            ...
            # Preserve the full dense index prefix during cold bootstrap.
            sparse_selection_explicit = selected_token_idx is not None
            deep_seen = {}
